  ...
  return (
    <div className="mx-auto max-w-[1400px] px-8 py-6" onClick={() => { if (menu || confirm) { setMenu(null); setConfirm(null); } }}>
      <BomPageHead
        ...
        onBack={onBack}
        actions={<button onClick={() => openDraft(null)} className={priBtnCls}><Plus size={14} /> Create Schedule</button>}
      />
      ...
      <div className={`mt-3 flex items-center gap-3 rounded-lg border px-4 py-3 ${enabled ? 'border-[#E5E7EB] bg-white' : 'border-[#F0E1BD] bg-[#FEFBF3]'}`}>
        ...
        <div className="flex-none text-right text-[12px] text-[#7B8FA5]">Last scheduler execution<br /><b className="mt-0.5 inline-block font-semibold text-[#364658]">Today · 02:00 IST</b></div>
      </div>

      <div className="mt-[22px] rounded-lg border border-[#E5E7EB] bg-white">
        <div className="flex items-center gap-2.5 border-b border-[#E5E7EB] px-4 py-3">
          <span className="text-[15px] font-semibold text-[#364658]">Schedule policies</span>
          <span className="inline-flex h-5 min-w-[20px] items-center justify-center rounded-full bg-[#EEF2F6] px-1.5 text-[12px] font-semibold tabular-nums text-[#64748B]">{store.schedules.length}</span>
          <div className="relative ml-auto w-[230px]">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#9CA3AF]" size={14} />
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search policies..." className="h-8 w-full rounded border border-[#d1d5db] bg-white pl-7 pr-7 text-[13px] text-[#364658] placeholder:text-[#9CA3AF] focus:border-[#3D8BD0] focus:outline-none" />
        ...
        {store.schedules.length === 0 ? (
          <div className="flex flex-col items-center px-5 py-[72px] text-center">
            ...
            <p className="mt-[5px] max-w-[440px] text-[13px] leading-[1.55] text-[#7B8FA5]">Nothing is generated automatically. Create a policy to have the agent build BOMs for enrolled CIs on a cadence you choose.</p>
          ...
          <div className="flex flex-col items-center px-5 py-[72px] text-center">
            ...
            <p className="mt-[5px] max-w-[440px] text-[13px] leading-[1.55] text-[#7B8FA5]">That is a filter result, not an all-clear — {store.schedules.length} policies still exist.</p>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-[#E5E7EB] text-[12px] font-semibold tracking-[.05em] text-[#364658]">
                <th className="w-[26%] px-4 py-[11px]">Policy</th><th className="px-4 py-[11px]">Coverage</th>
                <th className="px-4 py-[11px]">Trigger</th><th className="px-4 py-[11px]">Next Run</th>
                <th className="px-4 py-[11px]">Status</th><th className="w-[190px] px-4 py-[11px]" />
              </tr>
            </thead>
            <tbody>
              {list.map((p) => {
                ...
                return (
                  <tr key={p.id} className={`border-b border-[#F0F2F5] text-[13px] text-[#364658] last:border-b-0 ${p.on ? 'hover:bg-[#F9FAFB]' : 'bg-[#FCFDFE]'}`}>
                    <td className="px-4 py-3.5">
                      <div className={`font-semibold ${p.on ? 'text-[#364658]' : 'text-[#7B8FA5]'}`}>{p.name || bsEcho(p)}</div>
                      <div className="mt-[3px] font-mono text-[11px] text-[#9CA3AF]">{p.id}</div>
                    </td>
                    <td className="px-4 py-3.5">
                    ...
                    <td className="px-4 py-3.5">{bsTrigger(p)}<div className="mt-[3px] text-[12px] tabular-nums text-[#7B8FA5]">{p.time}</div></td>
                    <td className="px-4 py-3.5 font-medium">
                      {n
                        ? <>{bsWhen(n)}<div className="mt-[3px] text-[12px] font-normal tabular-nums text-[#7B8FA5]">{p.time}</div></>
                        : <span className="text-[#9CA3AF]">—<div className="mt-[3px] text-[12px] font-normal text-[#7B8FA5]">{enabled ? 'Paused' : 'Scheduling off'}</div></span>}
                    </td>
                    <td className="px-4 py-3.5"><StatusPill on={p.on} onLabel="Active" offLabel="Disabled" /></td>
                    <td className="relative px-4 py-3.5" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          className="inline-flex h-7 items-center gap-[5px] rounded border border-[#DFE5ED] bg-white px-2.5 text-[12px] font-medium text-[#364658] transition-colors hover:bg-[#F5F7FA]"
                        ><Play size={13} /> Run Now</button>
                        <button onClick={() => openDraft(p)} className="inline-flex h-7 items-center rounded border border-[#DFE5ED] bg-white px-2.5 text-[12px] font-medium text-[#364658] transition-colors hover:bg-[#F5F7FA]">Edit</button>
                        <button onClick={() => { setMenu(menu === p.id ? null : p.id); setConfirm(null); }}
                                aria-label={`More actions for ${p.name}`}
                                className="flex size-7 items-center justify-center rounded border border-[#DFE5ED] bg-white text-[#7B8FA5] transition-colors hover:bg-[#F5F7FA] hover:text-[#364658]"><MoreHorizontal size={15} /></button>
                      </div>
                      {menu === p.id && (
                        <div className="absolute right-4 top-12 z-[70] w-[190px] rounded-lg border border-[#DFE5ED] bg-white py-1 text-left shadow-[0_10px_15px_-3px_rgba(16,24,40,0.1),0_4px_6px_-4px_rgba(16,24,40,0.1)]">
                          <button onClick={() => { p.on = !p.on; setMenu(null); refresh(); toast(`"${p.name || bsEcho(p)}" ${p.on ? 'enabled' : 'disabled'}`); }}
                                  className="block w-full px-3 py-2 text-left text-[13px] text-[#364658] transition-colors hover:bg-[#F9FAFB]">{p.on ? 'Disable policy' : 'Enable policy'}</button>
                          <button
                            onClick={() => {
                            ...
                            className="block w-full px-3 py-2 text-left text-[13px] text-[#364658] transition-colors hover:bg-[#F9FAFB]"
                          >Duplicate policy</button>
                          <div className="my-1 border-t border-[#F0F2F5]" />
                          <button onClick={() => { setConfirm(p.id); setMenu(null); }}
                                  className="block w-full px-3 py-2 text-left text-[13px] text-[#DC2626] transition-colors hover:bg-[#FEF3F2]">Delete policy</button>
                        </div>
                      )}
                      {confirm === p.id && (
                        <div className="absolute right-4 top-12 z-[70] w-[280px] rounded-lg border border-[#DFE5ED] bg-white p-3 text-left shadow-[0_10px_15px_-3px_rgba(16,24,40,0.1),0_4px_6px_-4px_rgba(16,24,40,0.1)]">
                          ...
                          <p className="mt-1 text-[12px] leading-[1.5] font-normal text-[#7B8FA5]">
                          ...
                          <div className="mt-3 flex justify-end gap-2">
                            ...
                            <button
                              ...
                              className="inline-flex h-7 items-center rounded bg-[#DC2626] px-2.5 text-[12px] font-medium text-white transition-colors hover:bg-[#B91C1C]"
...
function PolicyDrawer({ draft, onChange, onClose, onSaved }: {
  ...
  return (
    <>
      ...
      >
        <div className="mb-2.5 text-[13px] font-semibold tracking-[-0.1px] text-[#364658]">Schedule setup</div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <div className="mb-1.5 text-[12px] font-medium text-[#7B8FA5]">Schedule Type <span className="text-[#DC2626]">*</span></div>
          ...
          {draft.kind === 'Recurring' && (
            <div>
              <div className="mb-1.5 text-[12px] font-medium text-[#7B8FA5]">Frequency <span className="text-[#DC2626]">*</span></div>
        ...
        {draft.kind === 'One Time' && (
          <div className="mt-3 max-w-[340px]">
            <div className="mb-1.5 text-[12px] font-medium text-[#7B8FA5]">Run on <span className="text-[#DC2626]">*</span></div>
        ...
        {draft.kind === 'Recurring' && draft.freq === 'Weekly' && (
          <div className="mt-3">
            <div className="mb-1.5 text-[12px] font-medium text-[#7B8FA5]">Days of the week <span className="text-[#DC2626]">*</span></div>
              ...
              {BS_DOW.map((d) => {
                ...
                return (
                  <button key={d} aria-pressed={on}
                          ...
                          className={`h-[34px] min-w-[46px] rounded border px-2.5 text-[13px] font-medium transition-colors ${on ? 'border-[#3D8BD0] bg-[#3D8BD0] text-white' : 'border-[#DFE5ED] bg-white text-[#364658] hover:bg-[#F5F7FA]'}`}>{d}</button>
                );
              })}
            </div>
            {!draft.days.length && <p className="mt-[5px] text-[12px] text-[#DC2626]">Pick at least one day.</p>}
          </div>
        )}
        {draft.kind === 'Recurring' && draft.freq === 'Monthly' && (
          <div className="mt-3 max-w-[340px]">
            <div className="mb-1.5 text-[12px] font-medium text-[#7B8FA5]">Day of month</div>
        ...
        <div className="mt-3 max-w-[340px]">
          <div className="mb-1.5 text-[12px] font-medium text-[#7B8FA5]">Start at</div>
        ...
        <div className="mt-3 rounded bg-[#F5FAFF] px-[13px] py-2.5 text-[13px] text-[#364658]">{bsEcho(draft)}</div>

        <div className="mb-[18px] mt-5 border-t border-[#F0F2F5]" />
        <div className="mb-2.5 text-[13px] font-semibold tracking-[-0.1px] text-[#364658]">Apply to CIs <span className="text-[12px] font-normal text-[#7B8FA5]">— by hand or by rule</span></div>
        <TargetTiles
        ...
        <div className="mb-[18px] mt-5 border-t border-[#F0F2F5]" />
        <div className="mb-1.5 text-[12px] font-medium text-[#7B8FA5]">Schedule name <span className="font-normal text-[#9CA3AF]">— optional</span></div>
